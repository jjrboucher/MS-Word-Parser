...
#
# This allows you to run this repeatedly against many DOCx file for an investiation and compare them.
...
#     In this worksheet, it will save the following information to a row:
#     "File Name", "Author", "Created Date","Last Modified By","Modified Date","Last Printed Date","Revision","Total Editing Time","Pages","Paragraphs","Lines","Words","Characters","Characters With Spaces","Title","Subject","Keywords","Description","Application","App Version","Template","Doc Security"
#
#
########## Dependencies ##########
#
...
def extract_from_apps_xml(xml_content):
    # a try/except is used for each otherwise the script will produce an error if one of the grep values is not in the file.
        try:
            ...
            template=""
        try:
            totalTime=re.search(r'<TotalTime>(.*?)</TotalTime>', xml_content).group(1)
        except:
            totalTime=""
        try:
            pages=re.search(r'<Pages>(.*?)</Pages>', xml_content).group(1)
        except:
            pages=""
        try:
            words=re.search(r'<Words>(.*?)</Words>', xml_content).group(1)
        except:
            words=""
        try:
            characters=re.search(r'<Characters>(.*?)</Characters>', xml_content).group(1)
        except:
            characters=""
        try:
            application=re.search(r'<Application>(.*?)</Application>', xml_content).group(1)
        except:
            application=""
        try:
            docSecurity=re.search(r'<DocSecurity>(.*?)</DocSecurity>', xml_content).group(1)
        except:
            docSecurity=""
        try:
            lines=re.search(r'<Lines>(.*?)</Lines>', xml_content).group(1)
        except:
            lines=""
        try:
            paragraphs=re.search(r'<Paragraphs>(.*?)</Paragraphs>', xml_content).group(1)
        except:
            paragraphs=""
        try:
            charactersWithSpaces=re.search(r'<CharactersWithSpaces>(.*?)</CharactersWithSpaces>', xml_content).group(1)
        except:
            characters=""
        try:
            appVersion=re.search(r'<AppVersion>(.*?)</AppVersion>', xml_content).group(1)
        except:
            appVersion=""
...
def extract_from_core_xml(xml_content):
        ...
        try:
            ...
            title=""
        try:
            subject=re.search(r'<dc:subject>(.*?)</dc:subject>', xml_content).group(1)
        except:
            subject=""
        try:
            creator=re.search(r'<dc:creator>(.*?)</dc:creator>', xml_content).group(1)
        except:
            creator=""
        try:
            keywords=re.search(r'<cp:keywords>(.*?)</cp:keywords>', xml_content).group(1)
        except:
            keywords=""
        try:
            description=re.search(r'<dc:description>(.*?)</dc:description>', xml_content).group(1)
        except:            
            description=""
        try:
            revision=re.search(r'<cp:revision>(.*?)</cp:revision>', xml_content).group(1)
        except:
            revision=""
        try:
            created=re.search(r'<dcterms:created.*?>(.*?)</dcterms:created>', xml_content).group(1)
        except:
            created=""
        try:
            modified=re.search(r'<dcterms:modified.*?>(.*?)</dcterms:modified>', xml_content).group(1)
        except:
            modified=""
        try:
            lastModifiedBy=re.search(r'<cp:lastModifiedBy>(.*?)</cp:lastModifiedBy>', xml_content).group(1)
        except:
            lastModifiedBy=""
        try:
            lastPrinted=re.search(r'<cp:lastPrinted>(.*?)</cp:lastPrinted>', xml_content).group(1)
        except:
            lastPrinted=""
	    
        return title, subject, creator, keywords, description, lastModifiedBy, revision, created, modified, lastPrinted
...
def write_to_excel(xml_files,excel_file_path, file_name, rsids, rsidRoot, creator, created, lastModifiedBy, modified, lastPrinted, revision, totalTime, pages, paragraphs, lines, words, characters, charactersWithSpaces, title, subject, keywords, description, application, appVersion, template, docSecurity):
    try:
        ...
        worksheet.append([file_name,len(rsids)-1,rsidRoot])
        ...
        if "metadata" in workbook.sheetnames: # if the worksheet metadata already exists, select it.
            ...
            worksheet.append(["File Name", "Author", "Created Date","Last Modified By","Modified Date","Last Printed Date","Revision","Total Editing Time","Pages","Paragraphs","Lines","Words","Characters","Characters With Spaces","Title","Subject","Keywords","Description","Application","App Version","Template","Doc Security"])

        worksheet.append([file_name,creator,created,lastModifiedBy,modified,lastPrinted,revision,totalTime,pages,paragraphs,lines,words,characters,charactersWithSpaces,title,subject,keywords,description,application,appVersion,template,docSecurity])
...
if __name__ == "__main__":
    ...
    zip_file_path = filedialog.askopenfilename(title="Select ZIP File", filetypes=[("ZIP Files", "*.docx")])
    if not zip_file_path:
        ...
        XMLFiles=list_of_XML_files(zip_file_path,filename) # Executes the function to get a list of all XML files within the DOCx file
        ...
        try:
            with zipfile.ZipFile(zip_file_path, 'r') as zip_ref:
                with zip_ref.open(xml_file_path_within_zip) as xml_file:
                    ...
                    rsids, rsidRoot = extract_rsids_from_xml(xml_content) # Executes the function to get all unique RSIDs.
        ...
        try:
            with zipfile.ZipFile(zip_file_path, 'r') as zip_ref:
                with zip_ref.open(xml_file_path_within_zip) as xml_file:
                    ...
                    template, totalTime, pages, words, characters, application, docSecurity, lines, paragraphs, charactersWithSpaces, appVersion = extract_from_apps_xml(xml_content) # Executes the function to get metadata from app.xml
        ...
        try:
            with zipfile.ZipFile(zip_file_path, 'r') as zip_ref:
                with zip_ref.open(xml_file_path_within_zip) as xml_file:
                    ...
                    title, subject, creator, keywords, description, lastModifiedBy, revision, created, modified, lastPrinted = extract_from_core_xml(xml_content) # Executes the function to get the metadata from core.xml
                    # print("Template:",template,"Total Time:",totalTime,"Pages:",pages)
        ...
        write_to_excel(XMLFiles,excel_file_path, filename, rsids,rsidRoot, creator,created,lastModifiedBy,modified,lastPrinted,revision,totalTime,pages,paragraphs,lines,words,characters,charactersWithSpaces,title,subject,keywords,description,application,appVersion,template,docSecurity) # Executes the function to write everything to the Excel file.
